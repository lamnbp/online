...
L.Map.WOPI = L.Handler.extend({
	...
	_postMessageListener: function(e) {
		...
		if (!this._appLoaded) {
			window.app.console.error('CO Clone not loaded yet. Listen for App_LoadingStatus (Document_Loaded) event before using PostMessage API. Ignoring post message \'' + msg.MessageId + '\'.');
			return;
		}
