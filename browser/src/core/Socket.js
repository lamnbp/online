	...
	connect: function(socket) {
		...
		if (socket && (socket.readyState === 1 || socket.readyState === 0)) {
			...
			try {
				...
				this._map.fire('error', {msg: _('Oops, there is a problem connecting to %productName: ').replace('%productName', (typeof brandProductName !== 'undefined' ? brandProductName : 'CODE Clone (unbranded)')) + e, cmd: 'socket', kind: 'failed', id: 3});
				return;
			}
		}
		...
		if (textMsg.startsWith('coolserver ')) {
			...
			if (command.errorKind === 'limitreached' && !this.WasShownLimitDialog) {
				...
				textMsg = textMsg.replace(/{productname}/g, (typeof brandProductName !== 'undefined' ?
					brandProductName : 'CODE Clone (unbranded)'));
